                ...
                try:
                    ...
                    try:
                        ...
                        await message.guild.unban(
                            ...
                            reason=f"Unbanned by {message.author.name}",
                        )

                        # Remove from the banned list
                        self.storage.settings["guilds"][guild_id]["banned_users"].pop(
                            str(user_id), None
                        )
                        ...
                        await message.channel.send(
                            f"**Unbanned user with ID:** `{user_id}`."
                        )
                ...
                try:
                    ...
                    if is_valid_duration(duration):
                        ...
                        if is_integer(duration):
                            ban_duration = int(time.time()) + int(duration)
                        else:
                        ...
                        try:
                            ...
                            await message.guild.ban(
                                discord.Object(id=user_id), reason=reason
                            )

                            # Store ban information
                            self.storage.settings["guilds"][guild_id]["banned_users"][
                                str(user_id)
                            ] = {
                            ...
                            await message.channel.send(
                                f"**Banned user with ID:** `{user_id}`. **Reason:** `{reason}`."
                            )
                        ...
                        await message.channel.send(
                            self.invalid_duration.format(usage=self.usage)
                        )
                except ValueError as e:
                    await message.channel.send(str(e))
            else:
            ...
            if len(command) >= 2:
                try:
                    ...
                    try:
                        ...
                        if "banned_users" not in self.storage.settings["guilds"].get(
                            guild_id, {}
                        ):
                            self.storage.settings["guilds"].setdefault(guild_id, {})[
                                "banned_users"
                            ] = {}
